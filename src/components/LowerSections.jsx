...
export function LowerSections() {
  ...
  return (
    <>
      ...
      <section className="section" id="reviews"><SectionTitle kicker="Отзывы" title="Клиенты ценят понятный расчет и спокойную коммуникацию" /><div className="review-grid">{reviews.map(([name, text]) => <article className="review-card" key={name}><div className="stars" aria-label="Пять звезд">★★★★★</div><p>{text}</p><strong>{name}</strong></article>)}</div><p className="review-note">Перед публикацией рекламы замените карточки на отзывы с ваших площадок и добавьте ссылки на источники.</p></section>
